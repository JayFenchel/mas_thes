...
Wobei
\begin{equation}
	\hat{d}_{k}=\frac{1}{\hat{h}_{k}-\hat{p}_{k}(z)}
\end{equation}
...
\end{equation}
Die Berechnungen der Ableitungen haben also analog Struktur wie \cite{BOY10} bis auf den zusätzlichen Term, der bei der Hessian hinzukommt.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{equation}
 ...
 =\begin{bmatrix}h_{i}\\0\end{bmatrix}_{k} - \begin{bmatrix}p_{i}^{T}\\
...
\begin{equation}
 \nabla f_{k}\left (z  \right )=\begin{bmatrix}p_{i}^{T}\\ -2\left(\left(c_{j}^{T}z + d_{j}\right )c_{j} - A_{j}^{T} \left(A_{j}z + b_{j} \right )\right)\end{bmatrix}_{k}
\end{equation}
\begin{equation}
