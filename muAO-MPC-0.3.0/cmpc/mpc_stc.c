...
void stc_ctl_warmstart(struct mpc_ctl *ctl) {
	...
	mpc_warmstart_vector(ctl->alm->fgm, ctl->alm->l_0, ctl->l_opt, ctl->alm->MXCONSTRS);
		...
		real_t u_opt[], real_t l_opt[])
{
	...
	real_t u_i[alm->fgm->HOR_INPUTS]; /* iteration update of input */
	real_t l_i[alm->HOR_MXCONSTRS]; /* it. update of Lagrange mult.*/

	uint32_t i;
	const size_t sizeof_u = alm->fgm->HOR_INPUTS * sizeof(*u_opt);
	const size_t sizeof_l = alm->HOR_MXCONSTRS * sizeof(*l_opt);
...
{
	real_t scb_x[alm->HOR_MXCONSTRS];

	mtx_multiply_mtx_vec(scb_x, alm->Kx_Ai, x, alm->HOR_MXCONSTRS, alm->fgm->STATES);
	mtx_substract(out, bound, scb_x, alm->HOR_MXCONSTRS, 1);
...
static void stc_fgm_minimize_qp(const struct mpc_alm *alm, real_t u[],
		const real_t u_i[], const real_t l_i[])
{
	real_t u_old[alm->fgm->HOR_INPUTS];
	real_t w[alm->fgm->HOR_INPUTS];
	real_t gradoL[alm->fgm->HOR_INPUTS];
	uint32_t i;
	const size_t sizeof_u = alm->fgm->HOR_INPUTS * sizeof(*u);
...
static void stc_fgm_compute_grad_over_L(const struct mpc_alm *alm,
		real_t gradoL[], const real_t w[], const real_t l_i[])
{
	real_t gradoL_inc[alm->fgm->HOR_INPUTS]; /* input constrained grad. over L*/
	real_t grad_stc[alm->fgm->HOR_INPUTS]; /* state constrained grad. */
	real_t gradoL_stc[alm->fgm->HOR_INPUTS]; /* state constrained grad. over L*/

	inc_fgm_compute_grad_over_L(alm->fgm, gradoL_inc, w);
	stc_fgm_compute_grad_alm(alm, grad_stc, w, l_i);
	mtx_scale(gradoL_stc, grad_stc, *(alm->Linv), alm->fgm->HOR_INPUTS, 1);
	mtx_add(gradoL, gradoL_inc, gradoL_stc, alm->fgm->HOR_INPUTS, 1);
		...
		const real_t l_i[], const real_t zx_b[])
{
	...
	real_t E_w[alm->HOR_MXCONSTRS];
	real_t diff[alm->HOR_MXCONSTRS];
	real_t pen_diff[alm->HOR_MXCONSTRS];

	mtx_multiply_mtx_vec(E_w, alm->E, w, alm->HOR_MXCONSTRS, alm->fgm->HOR_INPUTS);
	mtx_substract(diff, E_w, zx_b, alm->HOR_MXCONSTRS, 1);
	mtx_scale(pen_diff, diff, *(alm->mu), alm->HOR_MXCONSTRS, 1);
	mtx_add(out, l_i, pen_diff, alm->HOR_MXCONSTRS, 1);
...
static void stc_fgm_compute_l_update(const struct mpc_alm *alm,
		real_t l_i1[], const real_t w[], const real_t l_i[])
{
	real_t p_pos[alm->HOR_MXCONSTRS]; /* positive penalty */
	real_t p_neg[alm->HOR_MXCONSTRS]; /* negative penalty */
	...
	mtx_max_vec_zero(p_pos, alm->HOR_MXCONSTRS);
	mtx_min_vec_zero(p_neg, alm->HOR_MXCONSTRS);

	mtx_add(l_i1, p_pos, p_neg, alm->HOR_MXCONSTRS, 1);
...
static void stc_fgm_compute_grad_alm(const struct mpc_alm *alm,
		real_t out[], const real_t w[],	const real_t l_i[])
{
	real_t l_grad[alm->HOR_MXCONSTRS];
	real_t E_T[alm->HOR_MXCONSTRS * alm->fgm->HOR_INPUTS];

	stc_fgm_compute_l_update(alm, l_grad, w, l_i);
	mtx_transpose(E_T, alm->E, alm->HOR_MXCONSTRS, alm->fgm->HOR_INPUTS);
	mtx_multiply_mtx_vec(out, E_T, l_grad, alm->fgm->HOR_INPUTS, alm->HOR_MXCONSTRS);
