...
void mpc_ctl_solve_problem(struct mpc_ctl *ctl, const real_t x[])
{
	if ((ctl->x_ref == NULL) || (ctl->u_ref == NULL)) { /* any pointer is zero */
		if (ctl->alm->HOR_MXCONSTRS == NO_MXCONSTR) {
...
{
	real_t Adx[ctl->alm->fgm->STATES], Bdu[ctl->alm->fgm->STATES];

	/* compute x = Ad * x + Bd * u_opt; it overwrites x */
	mtx_multiply_mtx_vec(Adx, ctl->sys->Ad, x, ctl->alm->fgm->STATES, ctl->alm->fgm->STATES);
	mtx_multiply_mtx_vec(Bdu, ctl->sys->Bd, ctl->u_opt, ctl->alm->fgm->STATES, ctl->alm->fgm->INPUTS);
	mtx_add(x, Adx, Bdu, ctl->alm->fgm->STATES, 1);
...
void mpc_generate_state_trj(struct mpc_ctl *ctl, real_t x[])
{
  ...
  const size_t sizeof_x = ctl->alm->fgm->STATES * sizeof(*x);
  const size_t sizeof_u = ctl->alm->fgm->INPUTS * sizeof(*(ctl->u_opt));
  ...
  for (k=0; k<ctl->alm->fgm->HOR; k++) {
    /* compute x_k1 = Ad*x_k + Bd*u_k, k=0,...,N-1 */
    x_k1 = &(ctl->x_trj[(k+1)*ctl->alm->fgm->STATES]);
    x_k = &(ctl->x_trj[k*ctl->alm->fgm->STATES]);
    u_k = &(ctl->u_opt[k*ctl->alm->fgm->INPUTS]);

	mtx_multiply_mtx_vec(Adx, ctl->sys->Ad, x_k, ctl->alm->fgm->STATES, ctl->alm->fgm->STATES);
	mtx_multiply_mtx_vec(Bdu, ctl->sys->Bd, u_k, ctl->alm->fgm->STATES, ctl->alm->fgm->INPUTS);
	mtx_add(x_k1, Adx, Bdu, ctl->alm->fgm->STATES, 1);
