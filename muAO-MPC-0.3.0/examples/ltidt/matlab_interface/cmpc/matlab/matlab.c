...
mxArray* matlab_create_sys(const struct mpc_ctl *data) {
    ...
    matlab_add_matrix(sys, "Ad_T", data->sys->Ad_T, 
        data->alm->fgm->STATES, 
        data->alm->fgm->STATES
    );

    matlab_add_matrix(sys, "Bd_T", data->sys->Bd_T, 
        data->alm->fgm->INPUTS, 
        data->alm->fgm->STATES
    );
...
mxArray* matlab_create_qpx(const struct mpc_ctl *data) {
    ...
    matlab_add_scalar(qpx, "HOR_STCONSTRS", data->qpx->HOR_STCONSTRS);
...
mxArray* matlab_create_alm(const struct mpc_ctl *data) {
    ...
    matlab_add_scalar(alm, "STCONSTRS", data->alm->STCONSTRS);
    matlab_add_scalar(alm, "HOR_INPUTS", data->alm->HOR_INPUTS);
    matlab_add_scalar(alm, "HOR_STCONSTRS", data->alm->HOR_STCONSTRS);

    return alm;
}

mxArray* matlab_create_ctl(const struct mpc_ctl *data) {
    mxArray* ctl = mxCreateStructMatrix(1, 1, 0, 0);

    mxArray* conf = matlab_create_conf(data);
    mxAddField(ctl, "conf");
    mxSetField(ctl, 0, "conf", conf);

    mxArray* alm = matlab_create_alm(data);
    mxAddField(ctl, "alm");
    mxSetField(ctl, 0, "alm", alm);

    mxArray* sys = matlab_create_sys(data);
    mxAddField(ctl, "sys");
    mxSetField(ctl, 0, "sys", sys);

    mxArray* wmx = matlab_create_wmx(data);
    mxAddField(ctl, "wmx");
    mxSetField(ctl, 0, "wmx", wmx);

    mxArray* qpx = matlab_create_qpx(data);
    mxAddField(ctl, "qpx");
...
void matlab_apply_alm(struct mpc_ctl *data, const mxArray* alm) {
    ...
    matlab_read_matrix(alm, "l_0", data->alm->l_0, data->alm->HOR_STCONSTRS, 1);
    matlab_read_matrix(alm, "zx_lb", data->alm->zx_lb, data->alm->HOR_STCONSTRS, 1);
    matlab_read_matrix(alm, "zx_ub", data->alm->zx_ub, data->alm->HOR_STCONSTRS, 1);
}

void matlab_apply_ctl(struct mpc_ctl *data, const mxArray* ctl) {
    mxArray* conf = mxGetField(ctl, 0, "conf");
    matlab_apply_conf(data, conf);

    mxArray* alm = mxGetField(ctl, 0, "alm");
    matlab_apply_alm(data, alm);

    matlab_read_matrix(ctl, "u_opt", data->u_opt, data->alm->fgm->HOR_INPUTS, 1);
    matlab_read_matrix(ctl, "l_opt", data->l_opt, data->alm->HOR_STCONSTRS, 1);
    matlab_read_matrix(ctl, "u_ini", data->u_ini, data->alm->fgm->HOR_INPUTS, 1);
    matlab_read_matrix(ctl, "l_ini", data->l_ini, data->alm->HOR_STCONSTRS, 1);
    ...
    if (mxGetField(ctl, 0, "x_ref")) {
        ...
        matlab_read_matrix(ctl, "x_ref", data->u_ref, data->alm->fgm->HOR_STATES, 1);
    }
}
