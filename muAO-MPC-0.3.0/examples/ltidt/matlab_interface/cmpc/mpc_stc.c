...
static void stc_compute_state_constr_bound_online(const struct mpc_alm *alm,
		real_t out[], const real_t x[],
		const real_t bound[]);
...
void stc_ctl_warmstart(struct mpc_ctl *ctl) {
	...
	mpc_warmstart_vector(ctl->alm->fgm, ctl->alm->l_0, ctl->l_opt, MPC_STCONSTRS);
		...
		real_t u_opt[], real_t l_opt[])
{
	...
	static real_t l_i[MPC_HOR_STCONSTRS]; /* it. update of Lagrange mult.*/

	uint32_t i;
	const size_t sizeof_u = MPC_HOR_INPUTS * sizeof(*u_opt);
	const size_t sizeof_l = MPC_HOR_STCONSTRS * sizeof(*l_opt);
		...
		const real_t l_i[], const real_t zx_b[])
{
	...
	static real_t E_w[MPC_HOR_STCONSTRS];
	static real_t diff[MPC_HOR_STCONSTRS];
	static real_t pen_diff[MPC_HOR_STCONSTRS];

	mtx_multiply_mtx_vec(E_w, alm->E, w, MPC_HOR_STCONSTRS, MPC_HOR_INPUTS);
	mtx_substract(diff, E_w, zx_b, MPC_HOR_STCONSTRS, 1);
	mtx_scale(pen_diff, diff, *(alm->mu), MPC_HOR_STCONSTRS, 1);
	mtx_add(out, l_i, pen_diff, MPC_HOR_STCONSTRS, 1);
...
static void stc_fgm_compute_l_update(const struct mpc_alm *alm,
		real_t l_i1[], const real_t w[], const real_t l_i[])
{
	static real_t p_pos[MPC_HOR_STCONSTRS]; /* positive penalty */
	static real_t p_neg[MPC_HOR_STCONSTRS]; /* negative penalty */
	...
	mtx_max_vec_zero(p_pos, MPC_HOR_STCONSTRS);
	mtx_min_vec_zero(p_neg, MPC_HOR_STCONSTRS);

	mtx_add(l_i1, p_pos, p_neg, MPC_HOR_STCONSTRS, 1);
...
static void stc_fgm_compute_grad_alm(const struct mpc_alm *alm,
		real_t out[], const real_t w[],	const real_t l_i[])
{
	static real_t l_grad[MPC_HOR_STCONSTRS];
	static real_t E_T[MPC_HOR_STCONSTRS * MPC_HOR_INPUTS];

	stc_fgm_compute_l_update(alm, l_grad, w, l_i);
	mtx_transpose(E_T, alm->E, MPC_HOR_STCONSTRS, MPC_HOR_INPUTS);
	mtx_multiply_mtx_vec(out, E_T, l_grad, MPC_HOR_INPUTS, MPC_HOR_STCONSTRS);
...
static void stc_compute_state_constr_bound_online(const struct mpc_alm *alm,
		real_t out[], const real_t x[], const real_t bound[])
{
	static real_t scb_x[MPC_HOR_STCONSTRS];

	mtx_multiply_mtx_vec(scb_x, alm->Kx_Ai, x, MPC_HOR_STCONSTRS, MPC_STATES);
	mtx_substract(out, bound, scb_x, MPC_HOR_STCONSTRS, 1);
